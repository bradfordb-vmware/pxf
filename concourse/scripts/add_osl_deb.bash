...
deb_file_name=$(find pxf_artifacts -type f -name "pxf-gp${GP_VER}_*-1.${TARGET_OS_VERSION}_amd64.deb" -printf "%f\n")
[[ -n ${deb_file_name} ]] || fail "pxf_artifacts/pxf-gp${GP_VER}_*-1.${TARGET_OS_VERSION}_amd64.deb not found"
...
mv rebuild.deb pxf-gp${GP_VER}_$(dpkg-deb --field rebuild.deb Version).${TARGET_OS_VERSION}_amd64.deb

# check if the new DEB for GP version and TARGET_OS_VERSION is available
new_deb_file_name=$(find ./ -type f -name "pxf-gp${GP_VER}_*-2.${TARGET_OS_VERSION}_amd64.deb" -printf "%f\n")
[[ -n ${new_deb_file_name} ]] || fail "./pxf-gp${GP_VER}_*-2.${TARGET_OS_VERSION}_amd64.deb not found"
